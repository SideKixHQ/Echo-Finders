...
cannot be built without changing what a story *is*, and a fourth is modelled in a way that
would cause real problems in production.

## 1. Long stories cannot be centred on the place they describe
The prototype's library runs from 3:10 to 15:22. The engine assumed 30s, 90s and 3½
minutes, and centred every story on the moment the listener was nearest its subject.

At cruise that breaks badly. A ten-minute story covers roughly 1,400km, so centring it
...
ten-minute story could never satisfy it, and **long-form content would simply never have
played**. The tests passed throughout, because no fixture had a long story in it.

**Decision:** a story's anchor — its "you are here" moment — is its midpoint up to four
minutes and a fixed one-minute lead beyond that (`anchorOffsetS`). Long stories begin
...
duration and script for each story. That is correct and worth protecting: it is not the same
...
**Decision:** `SimpleVariant` is a first-class part of a story, validated in CI, and
...
there is recorded audio, because the drift accumulates over a fifteen-minute story.
...
the story, which is the sort of bug nobody reports and everybody notices.

## 4. Advertising is not a story category
...
- "Submit a story" conflicts with ADR-0005, which keeps editorial control closed at launch.
